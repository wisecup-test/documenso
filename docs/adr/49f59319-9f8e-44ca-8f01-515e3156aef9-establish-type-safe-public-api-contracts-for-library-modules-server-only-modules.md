# Establish Type-Safe Public API Contracts for Library Modules: Server Only Modules
...
1. MUST: Server-only modules MUST be isolated in dedicated directories (e.g., server-only/) to prevent client-side imports

## Policy Block

- MUST Server-only modules MUST be isolated in dedicated directories (e.g., server-only/) to prevent client-side imports
