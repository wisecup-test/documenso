# Enforce Server-Only Code Isolation for Runtime Environment Boundaries: Client Side Code
...
1. MUST_NOT: Client-side code MUST NOT directly import from server-only modules; all interactions MUST occur through defined API boundaries (e.g., API routes, server actions, loaders)

## Policy Block

- MUST_NOT Client-side code MUST NOT directly import from server-only modules; all interactions MUST occur through defined API boundaries (e.g., API routes, server actions, loaders)
