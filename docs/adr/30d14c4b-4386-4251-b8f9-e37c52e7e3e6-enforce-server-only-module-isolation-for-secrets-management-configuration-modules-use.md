# Enforce Server-Only Module Isolation for Secrets Management: Configuration Modules Use
...
1. SHOULD: Configuration modules SHOULD use TypeScript path aliases to clearly distinguish server-only imports (e.g., '@/lib/server-only/*')

## Policy Block

- SHOULD Configuration modules SHOULD use TypeScript path aliases to clearly distinguish server-only imports (e.g., '@/lib/server-only/*')
