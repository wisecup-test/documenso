# Enforce Server-Only Code Isolation for Runtime Environment Boundaries: Server Only Modules
...
1. SHOULD: Server-only modules SHOULD be organized in dedicated directory structures (e.g., 'server-only/', 'server/', '.server.ts' suffix) to provide clear visual and structural boundaries

## Policy Block

- SHOULD Server-only modules SHOULD be organized in dedicated directory structures (e.g., 'server-only/', 'server/', '.server.ts' suffix) to provide clear visual and structural boundaries
