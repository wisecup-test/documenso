# Adopt Server-Only Data Access Layer with Specialized Query Functions: Each Data Access
...
1. MUST: Each data access function MUST be named according to its operation using the pattern: <verb>-<entity>[-qualifier].ts (e.g., get-field-by-id.ts, find-folders.ts, update-folder.ts)

## Policy Block

- MUST Each data access function MUST be named according to its operation using the pattern: <verb>-<entity>[-qualifier].ts (e.g., get-field-by-id.ts, find-folders.ts, update-folder.ts)
