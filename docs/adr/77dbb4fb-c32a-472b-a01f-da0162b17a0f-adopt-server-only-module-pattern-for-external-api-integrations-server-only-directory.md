# Adopt Server-Only Module Pattern for External API Integrations: Server Only Directory
...
1. SHOULD: Server-only directory structures SHOULD follow the pattern 'packages/lib/server-only/{domain}/' for clear organizational boundaries

## Policy Block

- SHOULD Server-only directory structures SHOULD follow the pattern 'packages/lib/server-only/{domain}/' for clear organizational boundaries
