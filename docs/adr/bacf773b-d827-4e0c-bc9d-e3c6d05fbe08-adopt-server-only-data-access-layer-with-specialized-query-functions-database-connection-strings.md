# Adopt Server-Only Data Access Layer with Specialized Query Functions: Database Connection Strings
...
1. MUST_NOT: Database connection strings, credentials, or raw SQL queries MUST NOT be exposed outside the server-only data access layer

## Policy Block

- MUST_NOT Database connection strings, credentials, or raw SQL queries MUST NOT be exposed outside the server-only data access layer
