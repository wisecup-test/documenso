# Implement Cache Layer for Public API Route Handlers: Cache Ttl Time
...
1. SHOULD: Cache TTL (Time To Live) SHOULD be configured based on data volatility: 5 minutes for frequently changing data (members, groups), 15 minutes for relatively static data (settings, folders)

## Policy Block

- SHOULD Cache TTL (Time To Live) SHOULD be configured based on data volatility: 5 minutes for frequently changing data (members, groups), 15 minutes for relatively static data (settings, folders)
