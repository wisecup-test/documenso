# Adopt Server-Only Module Pattern for External API Integrations: Build Tooling Enforce
...
1. MUST: Build tooling MUST enforce server-only module boundaries and prevent client-side bundling of server-only code

## Policy Block

- MUST Build tooling MUST enforce server-only module boundaries and prevent client-side bundling of server-only code
