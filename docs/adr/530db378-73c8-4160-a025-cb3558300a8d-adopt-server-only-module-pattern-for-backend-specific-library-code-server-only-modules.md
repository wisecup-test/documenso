...
1. MUST: Server-only modules MUST use the 'server-only' package or equivalent mechanism to throw runtime errors if imported in client environments

## Policy Block

- MUST Server-only modules MUST use the 'server-only' package or equivalent mechanism to throw runtime errors if imported in client environments
