# Implement Cache Layer for Public API Route Handlers: Cache Keys Include
...
1. MUST: Cache keys MUST include relevant scope identifiers (orgUrl, teamUrl, userId) to prevent data leakage between tenants

## Policy Block

- MUST Cache keys MUST include relevant scope identifiers (orgUrl, teamUrl, userId) to prevent data leakage between tenants
