# Enforce Server-Only Module Isolation for Secrets Management: Server Only Modules
...
1. MUST: Server-only modules MUST import the 'server-only' package at the top of the file to trigger build-time errors if accidentally imported by client code

## Policy Block

- MUST Server-only modules MUST import the 'server-only' package at the top of the file to trigger build-time errors if accidentally imported by client code
