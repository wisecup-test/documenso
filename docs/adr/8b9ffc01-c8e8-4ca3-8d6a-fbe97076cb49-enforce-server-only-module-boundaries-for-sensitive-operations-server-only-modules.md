# Enforce Server-Only Module Boundaries for Sensitive Operations: Server Only Modules
...
1. MUST: Server-only modules MUST include runtime guards or build-time markers (e.g., 'server-only' package import) to prevent client-side execution

## Policy Block

- MUST Server-only modules MUST include runtime guards or build-time markers (e.g., 'server-only' package import) to prevent client-side execution
