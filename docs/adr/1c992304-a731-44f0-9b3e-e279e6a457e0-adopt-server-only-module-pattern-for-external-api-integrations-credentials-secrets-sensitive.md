# Adopt Server-Only Module Pattern for External API Integrations: Credentials Secrets Sensitive
...
1. MUST: API credentials, secrets, and sensitive configuration MUST only be accessed within server-only module boundaries

## Policy Block

- MUST API credentials, secrets, and sensitive configuration MUST only be accessed within server-only module boundaries
