# Adopt Server-Only Data Access Layer with Specialized Query Functions: Database Access Operations
...
1. MUST: All database access operations MUST be encapsulated in dedicated modules within a server-only directory structure that prevents client-side bundling

## Policy Block

- MUST All database access operations MUST be encapsulated in dedicated modules within a server-only directory structure that prevents client-side bundling
