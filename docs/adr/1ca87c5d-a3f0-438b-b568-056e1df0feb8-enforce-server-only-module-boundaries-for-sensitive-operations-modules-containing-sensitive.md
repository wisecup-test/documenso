# Enforce Server-Only Module Boundaries for Sensitive Operations: Modules Containing Sensitive
...
1. MUST: All modules containing sensitive operations (database queries, authentication checks, audit logging, PDF generation) MUST be placed in server-only directories or marked with server-only indicators

## Policy Block

- MUST All modules containing sensitive operations (database queries, authentication checks, audit logging, PDF generation) MUST be placed in server-only directories or marked with server-only indicators
