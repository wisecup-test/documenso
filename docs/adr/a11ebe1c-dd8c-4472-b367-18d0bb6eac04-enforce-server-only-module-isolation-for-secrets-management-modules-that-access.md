# Enforce Server-Only Module Isolation for Secrets Management: Modules That Access
...
1. MUST: All modules that access environment variables containing secrets or credentials MUST be placed in directories explicitly marked as server-only (e.g., 'server-only' path segments)

## Policy Block

- MUST All modules that access environment variables containing secrets or credentials MUST be placed in directories explicitly marked as server-only (e.g., 'server-only' path segments)
