# Enforce Server-Only Code Isolation for Runtime Environment Boundaries: Server Only Code
...
1. MUST: Server-only code containing database operations, secret access, or privileged business logic MUST be placed in modules with explicit server-only designation (e.g., 'server-only' directory or 'use server' directive)

## Policy Block

- MUST Server-only code containing database operations, secret access, or privileged business logic MUST be placed in modules with explicit server-only designation (e.g., 'server-only' directory or 'use server' directive)
