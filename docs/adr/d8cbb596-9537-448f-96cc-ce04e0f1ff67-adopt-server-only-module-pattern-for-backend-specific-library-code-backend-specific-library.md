# Adopt Server-Only Module Pattern for Backend-Specific Library Code: Backend Specific Library
...
1. MUST: All backend-specific library code that accesses databases, file systems, or server-only APIs MUST be placed in a 'server-only' module namespace

## Policy Block

- MUST All backend-specific library code that accesses databases, file systems, or server-only APIs MUST be placed in a 'server-only' module namespace
