<rule_activation id="d568782e-1dcf-43f9-aae3-5c3da09cd6d5" title="Adopt Structured Logging with Observability Context in Data Access Operations: Data Access Operations" applies_to="packages/lib/server-only/**/*.ts">
...
- **R-LOG-001** MAY: Data access operations MAY implement different log levels (DEBUG, INFO, WARN, ERROR) based on operational significance and environment configuration.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All data access operations in server-only modules MUST be instrumented with structured logging. Violations block deployment.
</enforcement>