<rule_activation id="1ca87c5d-a3f0-438b-b568-056e1df0feb8" title="Enforce Server-Only Module Boundaries for Sensitive Operations" applies_to="**/*">
...
- **R-SOOB-001** MUST: All modules containing sensitive operations (database queries, authentication checks, audit logging, PDF generation) MUST be placed in server-only directories or marked with server-only indicators.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Server-only boundary violations are security-critical and MUST be caught before code review approval or merge.
</enforcement>