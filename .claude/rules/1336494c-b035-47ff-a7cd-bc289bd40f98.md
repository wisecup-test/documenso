<rule_activation id="1336494c-b035-47ff-a7cd-bc289bd40f98" title="Adopt Server-Only Data Access Layer with Transactional Boundaries: Database Access Operations" applies_to="**/*">
...
- **R-SODAL-001** MUST: All database access operations MUST be implemented in the server-only layer and MUST NOT be directly accessible from client-side code.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All database access patterns MUST be validated against the server-only layer requirement before accepting code changes.
</enforcement>