<rule_activation id="8e71140b-93f1-47ea-ac7f-4de8036dfd50" title="Adopt E2E API Testing for Frontend Rendering Model Validation: E2e Tests Validate" applies_to="**/*">
...
- **R-E2E-001** MUST: E2E API tests MUST validate complete request-response cycles including authentication, authorization, data retrieval, and response formatting.
...
<enforcement>
Claude Code MUST verify E2E API test coverage on every pull request affecting API endpoints. Verification is mandatory and MUST NOT be skipped or deferred. Pull requests without corresponding E2E tests for API changes MUST be blocked from merging. CI/CD pipeline MUST fail if E2E test suite does not pass.
</enforcement>