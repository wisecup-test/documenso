<rule_activation id="52f1f85d-0439-4d6b-b35b-def5785f8bf8" title="Enforce Server-Only Module Isolation for Secrets Management: Server Only Modules" applies_to="**/*">
...
- **R-SECRETS-001** MUST: Server-only modules MUST import the 'server-only' package at the top of the file to trigger build-time errors if accidentally imported by client code.
...
<enforcement>
Claude Code MUST NOT skip or defer verification of server-only module isolation. All modules handling secrets, environment variables, or privileged operations MUST be verified to contain the 'server-only' import and reside in appropriate server-only directories.
</enforcement>