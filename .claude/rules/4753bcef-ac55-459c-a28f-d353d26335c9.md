<rule_activation id="4753bcef-ac55-459c-a28f-d353d26335c9" title="Standardize Structured Logging with Context Propagation for API and Server Operations: Error Logs Include" applies_to="**/*">
These rules are ALWAYS ACTIVE for all logging operations in API endpoints, server-only functions, and service clients.
</rule_activation>

### Rules

- **R-LOG-001** SHOULD: Error logs SHOULD include stack traces and contextual data sufficient for debugging without requiring log aggregation from multiple sources.
...
<enforcement>
Claude Code MUST NOT skip or defer verification of structured logging compliance. All logging in API endpoints and server-only functions MUST follow these standards.
</enforcement>