<rule_activation id="3da48768-c164-4ecc-9269-af6bfdbb0923" title="Standardize Structured Logging with Context Propagation for API and Server Operations: Log Entries Use" applies_to="**/*">
These rules are ALWAYS ACTIVE for all logging operations in API endpoints, server-only functions, and service clients.
</rule_activation>

### Rules

- **R-LOG-001** MUST: Log entries MUST use structured formats (JSON) rather than plain text to enable automated parsing and analysis.
...
<enforcement>
Claude Code MUST NOT skip or defer verification of structured logging compliance. All logging operations MUST use structured JSON formats and MUST NOT contain sensitive data patterns.
</enforcement>