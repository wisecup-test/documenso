<rule_activation id="ef0ca6a2-0e57-463d-8889-2fd7d4ee1144" title="Adopt Structured Logging with Observability Context in Data Access Operations: Data Access Operations" applies_to="packages/lib/server-only/**/*.ts">
...
- **R-DAL-001** MUST: All data access operations in server-only modules MUST implement structured logging with contextual information including operation type, entity identifiers, and timing data.
...
<enforcement>
Claude Code MUST NOT skip or defer verification of structured logging compliance in data access operations. All data access code in server-only modules MUST be instrumented with contextual logging before approval.
</enforcement>