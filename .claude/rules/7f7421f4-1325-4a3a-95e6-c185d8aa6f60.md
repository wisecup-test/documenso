<rule_activation id="7f7421f4-1325-4a3a-95e6-c185d8aa6f60" title="Enforce Server-Only Module Boundaries for Sensitive Operations: Server Only Modules" applies_to="**/*">
...
- **R-SOB-001** SHOULD: Server-only modules SHOULD be organized in dedicated directories (e.g., /server-only/, /api/, /lib/server/) to provide clear visual separation.
...
<enforcement>
Claude Code MUST verify server-only module boundaries are enforced via automated CI checks, bundle analysis, and ESLint rules. Violations MUST block PR merges. Verification is mandatory and MUST NOT be skipped or deferred.
</enforcement>