<rule_activation id="7b28c28f-0dd3-4681-8142-6d2fe8b3ac5e" title="Standardize tRPC Router Pattern for Primary Datastore Access: Datastore Access Logic" applies_to="**/*">
...
- **R-TRPC-001** MUST: Datastore access logic within tRPC routers MUST be encapsulated in dedicated handler functions or service layer methods, not inline within router definitions.
...
# Find router procedures without service layer imports
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Violations are caught by automated CI checks, TypeScript compilation, code review process, and periodic architecture audits. Non-compliant patterns block pull requests until refactored or approved as exceptions.
</enforcement>